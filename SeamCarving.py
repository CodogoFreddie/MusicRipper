...
def compute_min_energy(img):
    ...
    yRange = range(0,width-1)
    for i in xrange(1,height):
        for j in yRange:
        	out[i,j] = img[i,j] + min(out[i-1,j-1], out[i-1,j], out[i-1,j+1])

        out[i,0] = img[i,0] + min(out[i-1,0], out[i-1,0+1])
        out[i,width-1] = img[i,width-1] + min(out[i-1,width-2], out[i-1,width-1])
...
def main(infile, outfile, desired_w, desired_h):
    start = time.time()
